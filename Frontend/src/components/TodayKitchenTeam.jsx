...
const TodayKitchenTeam = () => {
     ...
     const fetchData = async () => {
        try {
            const [menuRes, studentsRes, feedbackRes, topTeamsRes, photosRes] = await axios.all([
                axios.get('https://kitchen-turn-project-1.onrender.com/menu/today'),
                axios.get('https://kitchen-turn-project-1.onrender.com/students/active'),
                axios.get('https://kitchen-turn-project-1.onrender.com/api/feedback'),
                axios.get('https://kitchen-turn-project-1.onrender.com/api/ratings/top-teams'),
                axios.get('https://kitchen-turn-project-1.onrender.com/api/photos')
            ]);
    ...
    useEffect(() => {
     ...
     const handleAddMealItem = (e) => {
        ...
        axios.patch(`https://kitchen-turn-project-1.onrender.com/menu/update-meal/${mealType}`, { foodName, nutrients })
            .then(() => fetchData())
    ...
    const handleRemoveMealItem = (mealType, itemId) => {
        axios.patch(`https://kitchen-turn-project-1.onrender.com/menu/remove-meal/${mealType}/${itemId}`)
            .then(() => fetchData())
    ...
    const handleFeedbackSubmit = (e) => {
        ...
        axios.post('https://kitchen-turn-project-1.onrender.com/api/feedback/add', newFeedback)
            .then(() => {
    ...
    const handleRateFood = (starValue) => {
        ...
        axios.post('https://kitchen-turn-project-1.onrender.com/api/ratings/add', { teamMembers, starValue })
            .then(() => {
    ...
    const handleResetStars = () => {
        if (window.confirm("Are you sure you want to reset all team ratings to zero? This action cannot be undone.")) {
            axios.post('https://kitchen-turn-project-1.onrender.com/api/ratings/reset')
                .then(() => {
    ...
    const handleFileChange = async (e) => {
        ...
        try {
            const uploadPromises = files.map(async (file) => {
                ...
                const response = await axios.post('https://kitchen-turn-project-1.onrender.com/api/upload-photo', formData, {
    ...
    const handleRemovePhoto = async (photoId) => {
        try {
            await axios.delete(`https://kitchen-turn-project-1.onrender.com/api/photos/${photoId}`);
            await fetchData();
