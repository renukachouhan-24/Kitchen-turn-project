...
const StudentDashboard = () => {
    ...
    const fetchAllStudents = async () => {
        try {
            const response = await axios.get('https://kitchen-turn-project-3.onrender.com/students/all');
            const sortedStudents = response.data.sort((a, b) => {
                if (a.status === 'on_leave' && b.status !== 'on_leave') return 1;
                if (a.status !== 'on_leave' && b.status === 'on_leave') return -1;
                return 0;
    ...
    const fetchActiveStudentsForTeams = async () => {
        try {
            const response = await axios.get('https://kitchen-turn-project-3.onrender.com/students/active');
            let activeStudents = response.data.filter(student => student.role !== 'coordinator');
    ...
    const handleSkipRotation = async () => {
        ...
        if (confirmSkip) {
            try {
                const response = await axios.post('https://kitchen-turn-project-3.onrender.com/api/skip-rotation');
    ...
    const handleStatusChange = async (studentId, newStatus) => {
        ...
        try {
            await axios.patch(`https://kitchen-turn-project-3.onrender.com/students/update-status/${studentId}`, { status: newStatus });
            fetchAllStudents();
            fetchActiveStudentsForTeams();
            setError(null);
        } catch (err) {
