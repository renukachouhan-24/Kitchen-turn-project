    ...
    const handleRoleChange = async (id, newRole) => {
        try {
            ...
            const res = await axios.patch(
                `https://kitchen-turn-project-1-mq2g.onrender.com/students/update-role/${id}`,
                { role: newRole },
            ...
            if (res.data.forceLogout && res.data.userId === currentUser._id) {
                const token = localStorage.getItem("token");
                const me = await axios.get("https://kitchen-turn-project-1-mq2g.onrender.com/api/auth/me", { // Corrected URL
                    headers: { Authorization: `Bearer ${token}` },
                });
