...
const UserManagement = () => {
    ...
    const fetchStudents = async () => {
        try {
            ...
            const response = await axios.get('https://kitchen-turn-project-1.onrender.com/students/all', {
    ...
    const handleRoleChange = async (id, newRole) => {
        try {
          const res = await axios.patch(
            `https://kitchen-turn-project-1.onrender.com/students/update-role/${id}`,
            { role: newRole },
          ...
          if (res.data.forceLogout && res.data.userId === currentUser._id) {
            const token = localStorage.getItem("token");
            const me = await axios.get("https://kitchen-turn-project-1.onrender.com/api/auth/me", {
              headers: { Authorization: `Bearer ${token}` },
            });
