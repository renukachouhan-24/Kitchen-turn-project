...
// FINAL CRON JOB - YEH ROZ RAAT 12 BAJE DATE CHANGE HONE PAR CHALEGA
cron.schedule('0 0 * * *', async () => {
  console.log('⏰ Running daily tasks at midnight when the date changes (India Time)...');
  
  try {
      // --- Task 1: Kitchen Team Rotation ---
      console.log('🔄 Performing daily kitchen team rotation...');
      const activeStudents = await Student.find({ status: 'active' }).sort({ turnOrder: 1 });
      const teamSize = 5;
      if (activeStudents.length >= teamSize) {
          const onDutyTeam = activeStudents.slice(0, teamSize);
          ...
          console.log(`✅ Team Rotation Successful.`);
      } else {
          console.log(`❌ Not enough active students for team rotation.`);
      }

      // --- Baaki ke Tasks (Data Reset, etc.) ---
      console.log('🧹 Performing daily data reset...');
      await Menu.deleteMany({});
      await Feedback.deleteMany({});
      await Photo.deleteMany({});
      
      console.log('🗑️ Deleting old resolved skip requests...');
      ...
      console.error("🚫 Error during scheduled daily tasks:", error);
  }
}, {
  scheduled: true,
